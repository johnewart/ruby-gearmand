module Rgearmand
  module ClientRequests
    ...
    def submit_job(func_name, uniq, data = nil, priority = "normal")
      job = worker_queue.enqueue(:priority => priority, :func_name => func_name, :uniq => uniq, :data => data, :persist => false)

      worker_queue.add(job.job_handle, self, job)

      logger.debug "Responding with JOB_CREATED"
    ...
    def submit_job_high(func_name, uniq, data)
      this.submit_job(func_name, uniq, data, "high")
    end

    # 32  SUBMIT_JOB_HIGH_BG  REQ    Client
    def submit_job_high_bg(func_name, uniq, data)
      job_handle = worker_queue.enqueue(:func_name => func_name, :uniq => uniq, :data => data, :persist => true, :priority => "high")
      respond :job_created, job_handle
    end

    # 33  SUBMIT_JOB_LOW      REQ    Client
    def submit_job_low(func_name, uniq, data)
      this.submit_job(func_name, uniq, data, "low")
    end

    # 34  SUBMIT_JOB_LOW_BG   REQ    Client
    def submit_job_low_bg(func_name, uniq, data)
      job_handle = worker_queue.enqueue(:func_name => func_name, :uniq => uniq, :data => data, :persist => true, :priority => "low")
      respond :job_created, job_handle
    end
